/*
 * File: drivers/input/keyboard/stmpe_keys.c
 * Description:  keypad driver for STMPE
...
struct stmpe_kpad {
	...
	const struct stmpe_kpad_platform_data *plat;
...
static inline int stmpe_read(struct stmpe_kpad *kpad, u8 reg)
{
	int ret;
...
static inline int stmpe_write(struct stmpe_kpad *kpad, u8 reg, u8 val)
{
	int ret;
...
static void keypad_backlight_on_slide()
{
	...
	if (!key_led_stat) {
		slide_state = gpio_get_value(gkpad->plat->slide_gpio);
		if (slide_state) {
			key_led_onoff(true);
			key_led_stat = true;
		}
...
static int stmpe1601_kpad_read_data(struct stmpe_kpad *kpad, u8 *data)
{
	int ret;
...
static void stmpe1601_message_read(void)
{
	...
	/* Key Read */
	val = stmpe_read(gkpad, STMPE1601_REG_INT_STA_LSB);
	...
	/* clear interrupt */
	stmpe_write(gkpad, STMPE1601_REG_INT_STA_LSB, val);
...
static int stmpe1801_kpad_read_data(struct stmpe_kpad *kpad, u8 *data)
{
	int ret;

	ret = i2c_smbus_read_i2c_block_data(kpad->i2c, STMPE1801_REG_KPC_DATA_BYTE0,
		STMPE1801_KPC_DATA_LEN, data);
	if (ret < 0)
		dev_err(&kpad->i2c->dev, "[%s]: failed to read key fifo !!!!!\n", __func__);
...
static void stmpe1801_message_read(void)
{
	u8 val, data, key_fifo[STMPE1801_KPC_DATA_LEN];
	int row, col, kcode;
	bool up;
	int slide_state;
	int i, ret;
	...
	/* Key Read */
	val = stmpe_read(gkpad, STMPE1801_REG_INT_STA_LOW);

	if (val & STPME1801_KPC_FIFO_OVR_EN)
		dev_err(&gkpad->i2c->dev, "[Q-KEY] Keypad FIFO Overflow\n");

	if (val & STPME1801_KPC_INT_EN) {
		ret = stmpe1801_kpad_read_data(gkpad, key_fifo);
		if (ret < 0)
			dev_err(&gkpad->i2c->dev, "[Q-KEY] Keypad data read failed\n");

		for (i = 0; i < STMPE1801_KPC_DATA_LEN-2; i++) {
			data = key_fifo[i];
			row = (data & STMPE1801_KPC_DATA_ROW) >> 3;
			col = data & STMPE1801_KPC_DATA_COL;
			kcode = MATRIX_SCAN_CODE(row, col, STMPE1801_KPAD_ROW_SHIFT);
			up = data & STMPE1801_KPC_DATA_UP;
			keypress = up;

			if ((data & STMPE1801_KPC_DATA_NOKEY_MASK) ==
				STMPE1801_KPC_DATA_NOKEY_MASK)
			    continue;
#if 0
			dprintk(KERN_DEBUG"[Q-KEY] Row = %d, Col = %d, UP = %d\n",
				row, col, up);
	...
	/* clear interrupt */
	stmpe_write(gkpad, STMPE1801_REG_INT_STA_LOW, val);
...
irqreturn_t stmpe_irq_handler(int irq, void *dev)
{
	if (system_rev > 6)
		stmpe1601_message_read();
	else
		stmpe1801_message_read();
...
{
	struct stmpe_kpad *kpad = (struct stmpe_kpad *)dev;
	struct input_dev *input = kpad->input;
	int lid_status = gpio_get_value(gkpad->plat->slide_gpio);
...
void stmpe_early_suspend(struct early_suspend *sus)
{
#if 0
	u8 val;
#endif
	...
	val = stmpe_read(gkpad, STMPE_REG_SYS_CTRL);
	stmpe_write(gkpad, STMPE_REG_SYS_CTRL,
		(val | STMPE_KPC_HYBER_EN));
#endif
}


void stmpe_late_resume(struct early_suspend *res)
{
	struct stmpe_kpad *kpad = container_of(res, struct stmpe_kpad,
			early_suspend);

	int lid_status;
	lid_status = gpio_get_value(kpad->plat->slide_gpio);
	...
	stmpe_read(gkpad, STMPE_REG_SYS_CTRL);
#endif
}
#endif

#ifdef STMPE_KPAD_DEBUG
static void stmpe1601_reg_dump(struct stmpe_kpad *kpad)
{
	u8 reg, reg_val[50];
	u8 reg_addr[] = {
...
static void stmpe1801_reg_dump(struct stmpe_kpad *kpad)
{
	u8 reg, reg_val[50];
	u8 reg_addr[] = {
		0x00, 0x01, 0x02, 0x04, 0x06, 0x08, 0x0a, 0x0b, 0x0c, 0x0d,
	...
	/* Chip Version */
	reg = 0x00;
	ret = i2c_smbus_read_i2c_block_data(kpad->i2c, reg, 3, &reg_val[0]);
	if (ret < 0)
		dev_err(&kpad->i2c->dev, "failed to read regs Reg[0x%02X]: %d\n",
			reg, ret);
	...
	/* System Regs */
	reg = 0x04;
	ret = i2c_smbus_read_i2c_block_data(kpad->i2c, reg, 1, &reg_val[3]);
	if (ret < 0)
		dev_err(&kpad->i2c->dev, "failed to read regs Reg[0x%02X]: %d\n",
			reg, ret);
	...
	/* Interrupt Regs */
	reg = 0x06;
	ret = i2c_smbus_read_i2c_block_data(kpad->i2c, reg, 1, &reg_val[4]);
	if (ret < 0)
		dev_err(&kpad->i2c->dev, "failed to read regs Reg[0x%02X]: %d\n",
			reg, ret);

	reg = 0x08;
		ret = i2c_smbus_read_i2c_block_data(kpad->i2c, reg, 1, &reg_val[5]);
		if (ret < 0)
			dev_err(&kpad->i2c->dev, "failed to read regs Reg[0x%02X]: %d\n",
				reg, ret);
	...
	/* Keypad Controller Regs */
	reg = 0x0a;
	ret = i2c_smbus_read_i2c_block_data(kpad->i2c, reg, 4, &reg_val[6]);
	if (ret < 0)
		dev_err(&kpad->i2c->dev, "failed to read regs Reg[0x%02X]: %d\n",
			reg, ret);
	...
	/* Keypad Controller Regs */
	reg = 0x30;
	ret = i2c_smbus_read_i2c_block_data(kpad->i2c, reg, 10, &reg_val[10]);
	if (ret < 0)
		dev_err(&kpad->i2c->dev, "failed to read regs Reg[0x%02X]: %d\n",
			reg, ret);
...
#if 0
static int __devinit stmpe_autosleep(struct stmpe_kpad *kpad,
	unsigned int autosleep_timeout)
{
	...
	for (i = 0; i < ARRAY_SIZE(stmpe_autosleep_delay); i++) {
		if (stmpe_autosleep_delay[i] >= autosleep_timeout) {
			timeout = i;
			break;
		}
	}
	...
	scw2 |= timeout | STPME_AUTOSLEEP_EN;
	ret = stmpe_write(kpad, STMPE_REG_SYS_CTRL2, scw2);
	if (ret < 0)
		return ret;
...
static int stmpe1601_endisable(struct stmpe_kpad *kpad,
	unsigned int cntrler, bool endis)
{
	...
	ret = stmpe_read(kpad, STMPE1601_REG_SYS_CTRL);
	if (ret < 0)
		return ret;
	...
	return stmpe_write(kpad, STMPE1601_REG_SYS_CTRL, ret);
}


static int __devinit stmpe1601_autosleep(struct stmpe_kpad *kpad,
	unsigned int autosleep_timeout)
{
	int timeout = -1;
	u8 scw2 = 0;
	...
	ret = stmpe_write(kpad, STMPE1601_REG_SYS_CTRL2, scw2);
	if (ret < 0)
		return ret;
...
static int __devinit stmpe1601_chip_init(struct stmpe_kpad *kpad)
{
	const struct stmpe_kpad_platform_data *plat = kpad->plat;
	u8 icw;
	int ret;
	...
	ret = stmpe_write(kpad, STMPE1601_GPIO_AF_U_MSB,
		STMPE1601_GPIO_ALT_Fn_KPAD);
	if (ret < 0)
		return ret;

	ret = stmpe_write(kpad, STMPE1601_GPIO_AF_U_LSB,
		STMPE1601_GPIO_ALT_Fn_KPAD);
	if (ret < 0)
		return ret;

	ret = stmpe_write(kpad, STMPE1601_GPIO_AF_L_MSB,
		STMPE1601_GPIO_ALT_Fn_KPAD);
	if (ret < 0)
		return ret;

	ret = stmpe_write(kpad, STMPE1601_GPIO_AF_L_LSB,
		STMPE1601_GPIO_ALT_Fn_KPAD);
	if (ret < 0)
		return ret;

	/* 2.Enabling scanning of columns */
	ret = stmpe_write(kpad, STMPE1601_REG_KPC_COL,
		STMPE1601_COL_SCAN_EN_WRD);
	if (ret < 0)
		return ret;

	/* 3.Enable scanning of rows */
	ret = stmpe_write(kpad, STMPE1601_REG_KPC_ROW_LSB,
		STMPE1601_ROW_SCAN_EN_WRD);
	if (ret < 0)
		return ret;

	/* 4.Setting scanning cycles */
	ret = stmpe_write(kpad, STMPE1601_REG_KPC_CTRL_MSB,
		plat->scan_count<<4);
	if (ret < 0)
		return ret;

	/* 5.Setting debounce and Scanning */
	ret = stmpe_write(kpad, STMPE1601_REG_KPC_CTRL_LSB,
	...
	ret = stmpe_write(kpad, STMPE1601_REG_INT_CTRL_LSB, icw);
	if (ret < 0)
		return ret;

	ret = stmpe_read(kpad, STMPE1601_REG_INT_EN_MASK_LSB);
	if (ret < 0)
		return ret;

	ret = stmpe_write(kpad, STMPE1601_REG_INT_EN_MASK_LSB,
...
static int __devinit stmpe1801_chip_init(struct stmpe_kpad *kpad)
{
	const struct stmpe_kpad_platform_data *plat = kpad->plat;
	int ret;
	u8 val[8] = {0, };
	...
	if (plat->debounce_ms > STMPE1801_KPAD_MAX_DEBOUNCE)
		return -EINVAL;

	if (plat->scan_count > STMPE1801_KPAD_MAX_SCAN_COUNT)
		return -EINVAL;

	regAdd=0x00;
	ret = i2c_smbus_read_i2c_block_data(kpad->i2c, regAdd, 3, &val[0]);
	mdelay(50);
	printk("Chip ID = %x %x %x\n" , val[0], val[1], val[2]);

	ret = stmpe_write(kpad, 0x02, 0x86);
	mdelay(20);

	regAdd=0x30;
	ret = i2c_smbus_read_i2c_block_data(kpad->i2c, regAdd, 6, &val[0]);
	mdelay(50);
	printk("Chip ID = %x %x %x %x %x %x\n" ,
		val[0], val[1], val[2], val[3], val[4], val[5]);

	ret = stmpe_write(kpad, 0x30, 0xff); /* KPC_ROW */
	mdelay(20);
	ret = stmpe_write(kpad, 0x31, 0xff); /* KPC_COL_LOW */
	mdelay(20);
	ret = stmpe_write(kpad, 0x32, 0x00); /* KPC_COL_HIGH */
	mdelay(20);
	ret = stmpe_write(kpad, 0x33, 0x50); /* KPC_CTRL_LOW */
	mdelay(20);
	ret = stmpe_write(kpad, 0x34, 0x3e); /* KPC_CTRL_MID */
	mdelay(20);
	ret = stmpe_write(kpad, 0x35, 0x43); /* KPC_CTRL_HIGH */
	mdelay(20);

	ret = stmpe_write(kpad, 0x06, 0x16); /* INT_ENABLE_MASK_LOW */
	mdelay(20);

	ret = stmpe_write(kpad, 0x04, 0x01); /* INT_CTRL_LOW */
	mdelay(20);

	ret = stmpe_write(kpad, 0x36, 0x01); /* KPC_CMD */
	mdelay(20);

	ret = stmpe_write(kpad, 0x19, 0xff); /* GPIO_SET_DIR_LOW */
	mdelay(20);

	ret = stmpe_write(kpad, 0x1c, 0xff); /* GPIO_RE_LOW */
	mdelay(20);

	ret = stmpe_write(kpad, 0x1f, 0xff); /* GPIO_FE_LOW */
	mdelay(20);

	ret = stmpe_write(kpad, 0x22, 0xff); /* GPIO_PULL_UP_LOW */
	mdelay(20);
/*
	ret = stmpe_write(kpad, 0x37, 0x07); // KPC_COMB_KEY_0
	mdelay(20);

	ret = stmpe_write(kpad, 0x38, 0x3f); // KPC_COMB_KEY_1
	mdelay(20);

	ret = stmpe_write(kpad, 0x39, 0x47); // KPC_COMB_KEY_2
	mdelay(20);
*/
	regAdd=0x30;
	ret = i2c_smbus_read_i2c_block_data(kpad->i2c, regAdd, 6, &val[0]);
...
{
	struct stmpe_kpad *kpad;
	struct stmpe_kpad_platform_data *pdata = client->dev.platform_data;
	struct input_dev *input;
	u8 revid, verid;
	int ret;
	...
	kpad = kzalloc(sizeof(struct stmpe_kpad), GFP_KERNEL);
	if (!kpad) {
	...
	printk("\n\n\n\nsystem_rev = %d\n\n\n\n", system_rev);
	if (system_rev > 6) {
		revid = stmpe_read(kpad, STMPE1601_REG_CHIP_ID);
		if (revid < 0) {
		...
		verid = stmpe_read(kpad, STMPE1601_REG_VERSION_ID);
		if (verid < 0) {
		...
		revid = stmpe_read(kpad, STMPE1801_REG_CHIP_ID);
		if (revid < 0) {
			printk(KERN_ERR"[Q-KEY] Not able to read revID\n");
			ret = revid;
			goto err1;
		...
		verid = stmpe_read(kpad, STMPE1801_REG_VERSION_ID);
		if (verid < 0) {
			printk(KERN_ERR"[Q-KEY] Not able to read verID\n");
			ret = verid;
			goto err1;
		...
		if (revid != STMPE1801_CHIP_ID) {
			printk(KERN_ALERT"[Q-KEY] device is not supported by this driver\n");
			return -EINVAL;
		}

		printk(KERN_INFO"[Q-KEY] ID=0x%02X, VER=0x%02X ### STMPE1801 detected\n",
			revid, verid);
	}

	input->name = client->name;
	input->phys = "stmpe-keys/input0";
	...
	matrix_keypad_build_keymap(pdata->keymap, STMPE_KPAD_ROW_SHIFT,
		input->keycode, input->keybit);

	if (system_rev > 6)
		ret = stmpe1601_chip_init(kpad);
	else
		ret = stmpe1801_chip_init(kpad);

	if (ret) {
		printk(KERN_ALERT "[Q-KEY] STMPE initialization failed\n");
		goto err1;
	}
	...
	ret = request_threaded_irq(kpad->i2c->irq, NULL, stmpe_irq_handler,
		kpad->plat->irq_trigger | IRQF_DISABLED, "stmpe_irq", kpad);
	if (ret) {
	...
	set_irq_type(pdata->slide_irq, IRQ_TYPE_EDGE_BOTH);
	...
	if (gpio_get_value(pdata->slide_gpio))
		input->sw[SW_LID] = 0;
	else
		input->sw[SW_LID] = 1;
#endif
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	kpad->early_suspend.level = EARLY_SUSPEND_LEVEL_DISABLE_FB + 1;
	kpad->early_suspend.suspend = stmpe_early_suspend;
	kpad->early_suspend.resume = stmpe_late_resume;
	register_early_suspend(&kpad->early_suspend);
#endif
	...
	if (system_rev > 6)
		...
		stmpe1801_reg_dump(kpad);
#endif

	printk(KERN_DEBUG"[Q-KEY] STMPE keypad driver registered\n");

	return 0;

err4:
#ifdef CONFIG_MACH_C1_NA_SPR_EPIC2_REV00
	free_irq(pdata->slide_irq, kpad);
#endif
err3:
...
err:
	...
	printk(KERN_DEBUG"[Q-KEY] STMPE keypad driver register fail\n");
...
{
	struct stmpe_kpad *kpad = i2c_get_clientdata(client);

	printk(KERN_DEBUG"[Q-KEY] %s\n", __func__);

	hrtimer_cancel(&kpad->timer);
#ifdef CONFIG_MACH_C1_NA_SPR_EPIC2_REV00
	free_irq(kpad->plat->slide_irq, kpad);
#endif
	free_irq(kpad->i2c->irq, kpad);
	...
	kfree(kpad);
#ifdef CONFIG_HAS_EARLYSUSPEND
	unregister_early_suspend(&kpad->early_suspend);
#endif
...
#ifdef CONFIG_PM
static int stmpe_suspend(struct i2c_client *client, pm_message_t mesg)
{
	struct stmpe_kpad_platform_data *pdata = client->dev.platform_data;
	...
	disable_irq(pdata->slide_irq);

	if (device_may_wakeup(&client->dev)) {
		enable_irq_wake(client->irq);
		enable_irq_wake(pdata->slide_irq);
	}

	g_slide_status = gpio_get_value(pdata->slide_gpio);

	return 0;
}


static int stmpe_resume(struct i2c_client *client)
{
	struct stmpe_kpad *kpad = i2c_get_clientdata(client);
	struct stmpe_kpad_platform_data *pdata = client->dev.platform_data;
	int lid_status;
	...
	lid_status = gpio_get_value(pdata->slide_gpio);

	printk(KERN_DEBUG"[Q-KEY] %s\n", __func__);

	if (device_may_wakeup(&client->dev)) {
		disable_irq_wake(client->irq);
		disable_irq_wake(pdata->slide_irq);
	}
	...
	enable_irq(pdata->slide_irq);

	return 0;
}
#else
#define stmpe_suspend	NULL
#define stmpe_resume	NULL
#endif
...
module_init(stmpe_init);
module_exit(stmpe_exit);

MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("STMPE KeyExpander Driver");
